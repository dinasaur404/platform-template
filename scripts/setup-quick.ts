...
function ensureDispatchNamespace(namespaceName: string) {
  ...
  try {
    ...
    const output =
      (error as { stdout?: string; stderr?: string })?.stdout?.toString() ||
...
async function createPermanentToken(config: Config): Promise<string | null> {
  ...
  try {
    ...
    const permGroupsData =
      await permGroupsResponse.json<
    ...
    const findPermGroup = (name: string) => {
    ...
    if (tokenResponse.ok && tokenData.success && tokenData.result?.value) {
      ...
      log(
        'yellow',
        `⚠️  Could not create token: ${
          tokenData.errors?.[0]?.message || 'Unknown error'
        }`
      );
      // Log more details for debugging
      if (tokenData.errors) {
        tokenData.errors.forEach((e) => {
    ...
    log('yellow', `⚠️  Token creation failed: ${(error as Error).message}`);
    return null;
  }
}

async function detectZoneId(config: Config): Promise<string | null> {
  ...
  try {
    ...
    log(
      'yellow',
      `⚠️  Could not auto-detect zone ID for ${config.customDomain}`
    );
    return null;
  } catch (error) {
    log('yellow', `⚠️  Zone detection failed: ${(error as Error).message}`);
    return null;
  }
}

function updateWranglerConfig(config: Config): boolean {
...
async function setWranglerSecrets(config: Config): Promise<void> {
  ...
  if (config.userApiToken) {
    try {
      ...
      log(
        'yellow',
        `⚠️  Could not set CLOUDFLARE_API_TOKEN secret: ${
  ...
  if (dispatchToken) {
    try {
      ...
      log(
        'yellow',
        `⚠️  Could not set DISPATCH_NAMESPACE_API_TOKEN secret: ${
          (error as Error).message
        }`
      );
...
async function main() {
  ...
  log(
    'cyan',
    `   CF_API_TOKEN: ${process.env.CF_API_TOKEN ? 'set' : 'not set'}`
  );
  log(
    'cyan',
  ...
  log('cyan', `   CUSTOM_DOMAIN (env): ${process.env.CUSTOM_DOMAIN}`);
  ...
  if (config.userApiToken) {
    log(
      'green',
      `   User API Token: ${config.userApiToken.substring(
        0,
        8
      )}... (will use for custom domains)`
    );
  ...
  // Write secrets to .env file - wrangler will use these during deploy
  ...
  if (dispatchToken || config.accountId || config.userApiToken) {
    ...
    if (config.userApiToken) {
      envContent += `CLOUDFLARE_API_TOKEN="${config.userApiToken}"\n`;
      log(
        'green',
    ...
    if (dispatchToken) {
      envContent += `DISPATCH_NAMESPACE_API_TOKEN="${dispatchToken}"\n`;
      log('green', `   ✅ DISPATCH_NAMESPACE_API_TOKEN (${tokenSource} token)`);
    }

    if (config.accountId) {
      envContent += `ACCOUNT_ID="${config.accountId}"\n`;
      log('green', `   ✅ ACCOUNT_ID`);
    }

    if (config.customDomain) {
      envContent += `CUSTOM_DOMAIN="${config.customDomain}"\n`;
    }

    if (config.zoneId) {
      envContent += `CLOUDFLARE_ZONE_ID="${config.zoneId}"\n`;
    }
    ...
    const fallbackOrigin =
      ...
      (config.customDomain ? `my.${config.customDomain}` : '');
    if (fallbackOrigin) {
      envContent += `FALLBACK_ORIGIN="${fallbackOrigin}"\n`;
    }
  ...
  if (setSecretsFlag) {
    ...
    // Read tokens from .env (saved during build phase)
    const envPath = path.join(PROJECT_ROOT, '.env');
    if (fs.existsSync(envPath)) {
      config.userApiToken =
        config.userApiToken || process.env.CLOUDFLARE_API_TOKEN || '';
      config.runtimeToken =
        config.runtimeToken ||
