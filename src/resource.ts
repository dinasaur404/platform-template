...
const BaseURI = `https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/workers`;

const ScriptsURI = () =>
  `${BaseURI}/dispatch/namespaces/${env.DISPATCH_NAMESPACE_NAME}/scripts`;
...
export async function PutScriptInDispatchNamespace(
  ...
  return await fetch(`${ScriptsURI()}/${scriptName}`, {
    method: 'PUT',
    body: formData,
    headers: {
      ...AuthHeaders
...
export async function PutScriptWithAssetsInDispatchNamespace(
  ...
  try {
    ...
    const sessionUrl = `${BaseURI}/dispatch/namespaces/${env.DISPATCH_NAMESPACE_NAME}/scripts/${scriptName}/assets-upload-session`;
    ...
    if (buckets && buckets.length > 0) {
      const uploadUrl = `https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/workers/assets/upload?base64=true`;
...
`;
    ...
    const deployResponse = await fetch(`${ScriptsURI}/${scriptName}`, {
      method: 'PUT',
      body: formData,
      headers: {
        ...AuthHeaders
